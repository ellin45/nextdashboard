...
const TeamList: React.FC<TeamListProps> = ({data,role}) => {
      ...
      <div>
        ...
        <div className="grid lg:grid-cols-3 md:grid-cols-2 gap-8">
          {approvedMembers.map((member, index) => (
            <TeamCard key={index} member={member} isPending={false} role={role}/>
          ))}
        </div>
      </div>
      <div>
        ...
        <div className="grid lg:grid-cols-3 md:grid-cols-2 gap-8">
          {pendingMembers.map((member, index) => (
            <TeamCard key={index} member={member} isPending={true} role={role}/>
          ))}
        </div>
      </div>
    </div>
